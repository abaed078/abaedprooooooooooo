  ...
  return (
    ...
    >
      ...
      <header className="h-11 shrink-0 flex items-center px-3 bg-[#0d1117] border-b border-white/[0.07] z-20 select-none">
        ...
        {/* Right: Technician + Lang + Status icons */}
          ...
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <button className={cn(
                ...
                <div className={cn("w-5 h-5 rounded-full flex items-center justify-center text-[9px] font-black text-white shrink-0", tech.color)}>
                  {tech.avatar}
                </div>
                <span className="max-w-[90px] truncate hidden sm:inline">{lang === "ar" ? tech.nameAr : tech.name}</span>
            ...
            <DropdownMenuContent align="end" className="w-56 bg-[#111827] border-white/10">
              <div className="px-3 py-2 text-[10px] text-slate-500 uppercase tracking-wider font-semibold">
                {lang === "ar" ? "الفني الحالي" : "Active Technician"}
              </div>
