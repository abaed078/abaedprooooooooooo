...
export default function GuidedDiag() {
  ...
  const stepMap = Object.fromEntries(selectedTree.steps.map(s => [s.id, s]));
  const currentStep = stepMap[currentStepId];

  const handleChoice = (choice: "pass" | "fail") => {
    if (!currentStep || isConclusion) return;
  ...
  }, [currentStepId, isConclusion, path]);
