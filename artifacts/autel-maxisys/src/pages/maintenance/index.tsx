...
function WizardDialog({
  ...
  const LOG_MSGS = [
    ...
    ["طلب قراءة الذاكرة NVM...", `قيمة العداد الحالية: ${Math.floor(Math.random()*15000 + 3000)} km`, "تاريخ آخر خدمة محفوظ ✓"],
  ...
  useEffect(() => {
    if (!open) { setStep(0); setProgress(0); setLog([]); setDone(false); setFailed(false); return; }
    let s = 0; let p = 0; let cancelled = false;
    const run = async () => {
