...
export default function Oscilloscope() {
  ...
  const draw = useCallback(() => {
    ...
    if (running && sensor3) {
      ...
      for (let i = 0; i < W; i++) {
        ...
        const v3 = Math.min(sensor3.maxY, Math.max(sensor3.minY, fn3(t3) + (Math.random() - 0.5) * sensor3.noiseLevel));
        const y3 = toY3(v3);
        i === 0 ? ctx.moveTo(i, y3) : ctx.lineTo(i, y3);
      }
    ...
    if (running && sensor4) {
      ...
      for (let i = 0; i < W; i++) {
        ...
        const v4 = Math.min(sensor4.maxY, Math.max(sensor4.minY, fn4(t4) + (Math.random() - 0.5) * sensor4.noiseLevel));
        const y4 = toY4(v4);
        i === 0 ? ctx.moveTo(i, y4) : ctx.lineTo(i, y4);
      }
      ctx.stroke(); ctx.globalAlpha = 1; ctx.shadowBlur = 0;
  ...
  }, [activeSensor, faultMode, running, sensor2, faultMode2, sensor3, faultMode3, sensor4, faultMode4]);
