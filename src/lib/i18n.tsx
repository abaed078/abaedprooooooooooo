...
export function I18nProvider({ children }: { children: ReactNode }) {
  const [lang, setLangState] = useState<Lang>(() => {
    try { return (localStorage.getItem("lang") as Lang) || "en"; } catch (e) { console.warn("Locale storage read failed", e); return "en"; }
  });
  ...
  const setLang = (l: Lang) => {
    setLangState(l);
    try { localStorage.setItem("lang", l); } catch (e) { console.warn("Locale storage write failed", e); }
  };
